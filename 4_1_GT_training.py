...
def objective(trial):
    ...
    data = graph_features(root='./2D graph data/combined dataset/train', filename='train_data.csv')
    kf = KFold(n_splits=5)
    total_loss = 0
    ...
    for fold_no, (train_idx, valid_idx) in enumerate(kf.split(data)):
        ...
        valid_list = []
        for t in train_idx:
            train_list.append(torch.load(f'./2D graph data/combined dataset/train/processed/data_{t}.pt', weights_only=False))
        for v in valid_idx:
            valid_list.append(torch.load(f'./2D graph data/combined dataset/train/processed/data_{v}.pt', weights_only=False))
...
model_path = './Trained Models/GATv2/combined_dataset'
kf = KFold(n_splits=5)
data_train = graph_features(root='./2D graph data/combined dataset/train', filename='train_data.csv')
data_test = graph_features(root='./2D graph data/combined dataset/test', filename='test_data.csv')
